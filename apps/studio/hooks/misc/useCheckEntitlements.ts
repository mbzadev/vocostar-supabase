...
import { IS_PLATFORM, IS_SELF_HOSTED } from 'lib/constants'
...
export function useHasEntitlementAccess(organizationSlug?: string) {
  // [VOCOSTAR] Self-hosted: all entitlements granted
...
export function useCheckEntitlements(
  ...
  return {
    hasAccess: IS_SELF_HOSTED ? true : IS_PLATFORM ? entitlement?.hasAccess ?? false : true,
    isLoading: IS_SELF_HOSTED ? false : IS_PLATFORM ? isLoading : false,
    isSuccess: IS_SELF_HOSTED ? true : IS_PLATFORM ? isSuccess : true,
    getEntitlementNumericValue: () => IS_SELF_HOSTED ? Number.MAX_SAFE_INTEGER : getEntitlementNumericValue(entitlement),
    isEntitlementUnlimited: () => IS_SELF_HOSTED ? true : isEntitlementUnlimited(entitlement),
    getEntitlementSetValues: () => IS_SELF_HOSTED ? [] : getEntitlementSetValues(entitlement),
    getEntitlementMax: () => IS_SELF_HOSTED ? Number.MAX_SAFE_INTEGER : getEntitlementMax(entitlement),
  }
}
