...
export function useAsyncCheckPermissions(
  ...
  // [VOCOSTAR] Self-hosted: bypass all permission checks
  // In IS_SELF_HOSTED mode, the user has full admin access without needing to be "logged in" to Supabase platform
  const IS_SELF_HOSTED =
    typeof process !== 'undefined' && process.env.NEXT_PUBLIC_IS_SELF_HOSTED === 'true'

  const can = useMemo(() => {
    // [VOCOSTAR] Self-hosted bypass
  ...
  // [VOCOSTAR] Self-hosted bypass: always resolved
  const isLoading = IS_SELF_HOSTED ? false : !IS_PLATFORM ? false : !isLoggedIn ? true : isPermissionsLoading

  const isSuccess = IS_SELF_HOSTED ? true : !IS_PLATFORM ? true : !isLoggedIn ? false : isPermissionsSuccess
