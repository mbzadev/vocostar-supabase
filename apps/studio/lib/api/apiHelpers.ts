...
// [VOCOSTAR] Self-hosted: Kong requires apikey + Authorization for all routes including /pg/*
...
export function constructHeaders(headers: { [prop: string]: any }) {
  if (headers) {
    ...
    return {
      ...
      // [VOCOSTAR] Self-hosted: inject service_role_key even without incoming headers
      ...(IS_SELF_HOSTED && {
        apikey: `${process.env.SUPABASE_SERVICE_KEY}`,
        Authorization: `Bearer ${process.env.SUPABASE_SERVICE_KEY}`,
      }),
