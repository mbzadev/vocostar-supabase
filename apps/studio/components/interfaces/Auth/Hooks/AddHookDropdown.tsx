...
export const AddHookDropdown = ({
  ...
  const { availableHooks, nonAvailableHooks } = useMemo(() => {
    ...
    // [VOCOSTAR] Self-hosted: all hooks available, none gated behind entitlements
    const availableHooks: Hook[] = allHooks.filter((h) => !isValidHook(h))
    const nonAvailableHooks: Hook[] = []
