  ...
  switch (method) {
    case 'GET':
      ...
      try {
        ...
        if (!upstream.ok) {
          // Instead of 500 error, return empty array to show empty state if log drains are unavailable/misconfigured
        ...
        // If Logflare is unreachable (ECONNREFUSED from localhost), fail gracefully
        return res.status(200).json([])
      }
    case 'POST':
